...
import com.inorain.playerlikeyoutubetest.transformer.Transformer;
import com.inorain.playerlikeyoutubetest.transformer.TransformerFactory;
...
public class DraggableView extends RelativeLayout {
    ...
    @Override public boolean onTouchEvent(MotionEvent ev) {
        ...
        if ((actionMasked & MotionEventCompat.ACTION_MASK) == MotionEvent.ACTION_DOWN) {
            ...
            Log.e(TAG," onTouchEvent == MotionEvent.ACTION_DOWN,,,activePointerId = "+activePointerId);
        }
        if (activePointerId == INVALID_POINTER) {
            Log.e(TAG," onTouchEvent activePointerId == INVALID_POINTER,,,activePointerId = "+activePointerId);
            return false;
        }
        viewDragHelper.processTouchEvent(ev);
        if (isClosed()) {
            Log.e(TAG," onTouchEvent isClosed()");
            return false;
        }
        ...
        if (isMaximized()) {
            ...
            Log.e(TAG," onTouchEvent isMaximized()");
        } else {
            Log.e(TAG," onTouchEvent !!!isMaximized()");
            dragView.dispatchTouchEvent(cloneMotionEventWithAction(ev, MotionEvent.ACTION_CANCEL));
        }
        Log.e(TAG," onTouchEvent return isDragViewHit = "+isDragViewHit+" isSecondViewHit = "+isSecondViewHit);
    ...
    private boolean isViewHit(View view, int x, int y) {
        ...
        Log.e(TAG," **screenX = "+screenX+" screenY = "+screenY);
        Log.e(TAG," viewLocation[0] = "+viewLocation[0]+" viewLocation[0] + view.getWidth() = "+viewLocation[0] + view.getWidth()+
                " viewLocation[1] = "+viewLocation[1]+"" +
                " viewLocation[1] + view.getHeight() = "+viewLocation[1] + view.getHeight());
        Log.e(TAG," compares >>> " +
                " screenX >= viewLocation[0] = "+(screenX >= viewLocation[0])+"" +
                " && screenX < viewLocation[0] + view.getWidth() = "+(screenX < viewLocation[0] + view.getWidth())+"" +
                " && screenY >= viewLocation[1] = "+(screenY >= viewLocation[1])+"" +
                " && screenY < viewLocation[1] + view.getHeight() = "+(screenY < viewLocation[1] + view.getHeight()));
